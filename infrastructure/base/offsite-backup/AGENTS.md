...
- The `appdata` job captures Audiobookshelf config and metadata, Jellyfin and Kavita config, and Tandoor media files as tarballs from their running app pods. Embedded application databases are part of those archives; PostgreSQL databases come from the `databases` job.
...
- Forgejo backups capture `/data` as a tarball from its running pod plus a fresh dump of the `forgejo` database. Forgejo is not quiesced: git objects are written before refs are updated, so a push during the backup leaves at most an unreferenced object. Do not add a maintenance window for it without a reason that outweighs the downtime.
