...
- Every job runs `restic unlock` before it locks. A pod that dies mid-run (deleted ad-hoc job, OOM kill, node reboot) leaves its lock file behind, and Restic only verifies the PID when the hostname matches — for a foreign pod name it never cleans up on its own. The result is not a single failed job but a permanent outage of all of them: `backup` still succeeds on its non-exclusive lock, while every `forget` burns through `--retry-lock` and fails. Never use `--remove-all` here; plain `unlock` removes only locks Restic itself deems stale, and a live Restic process keeps refreshing its own, so a genuinely overlapping job is never affected.
...
- The maintenance sidecar probes `/var/www/html/config` for writability first. A read-only remount of the Nextcloud iSCSI PVC (kernel flips RO after a TrueNAS/iSCSI I/O blip) makes `occ maintenance:mode --on` fail silently — this is the #1 cause of `WatchdogJobFailed` on `nextcloud-offsite-backup` and means Nextcloud itself is degraded. The probe fails the job fast with `maintenance-ro.failed` and an actionable message instead of 5 cryptic retries.
